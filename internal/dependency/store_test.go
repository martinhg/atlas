...
func (m *mockDepStore) SyncRepoDependencies(ctx context.Context, repoID uuid.UUID, deps []npm.ParsedDep) error {
...
func TestIntegration_ListByOrg_pagination(t *testing.T) {
	...
	// Create 5 deps.
	deps := make([]npm.ParsedDep, 5)
	for i := range deps {
		deps[i] = npm.ParsedDep{
			Ecosystem:  "npm",
