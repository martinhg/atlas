...
func NewHandler(orgStore OrgStore, catalogStore catalog.RepoStore, ghAppID int64, ghPrivateKey []byte, webhookSecret string) *Handler {
	return &Handler{
		orgStore:      orgStore,
...
func (h *Handler) HandleConnect(w http.ResponseWriter, r *http.Request) {
	...
	if h.catalogStore != nil {
		...
		if err != nil {
			...
			go syncRepos(installClient, h.orgStore, h.catalogStore, org.ID, org.Slug)
		}
	}
...
func (h *Handler) HandleGitHubWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if existing != nil {
		if h.catalogStore != nil {
			...
			if err == nil {
				go syncRepos(installClient, h.orgStore, h.catalogStore, existing.ID, existing.Slug)
			}
		}
		w.WriteHeader(http.StatusOK)
