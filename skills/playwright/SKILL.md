...
description: >
  ...
  Trigger: When writing Playwright E2E tests (Page Object Model, selectors, assertions, network mocking, auth state).
license: Apache-2.0
metadata:
  version: "1.0"
...
- `sign-up-critical-path.spec.ts` — WRONG: no separate files per concern
...
- Create page objects only when the page doesn't exist yet
- If a test requires multiple pages, ensure all page objects exist (create if needed)
...
test("Shows error on failed login", async ({ page }) => {
  ...
  const loginPage = new LoginPage(page);
  await loginPage.goto();
  await loginPage.login({ email: "bad@email.com", password: "wrong" });
  await loginPage.verifyNotificationMessage("Invalid credentials");
});

// Intercept and verify request payload
test("Sends correct payload on submit", async ({ page }) => {
...
- Focus ONLY on the specific test case
...
pnpm exec playwright test --grep "login"     # Filter by name
pnpm exec playwright test --ui               # Interactive UI
pnpm exec playwright test --debug            # Debug mode
pnpm exec playwright test tests/login/       # Run specific folder
pnpm exec playwright test --reporter=html    # HTML report
```
